...
local cp={} for k,v in pairs(component) do cp[k]=v end
...
component.invoke = function(address, method, ...)
	if address == fs.address then
		if method == "rename" then
			...
			if e[1] == "/OS.lua" then
				if _G.fileprotect(getCurrentScript() .. " Попытка переимовать/переместить загрузчик системы!") then return cp.invoke(address, method, ...) end
				return false
			elseif string.starts(e[1], "/Libraries/") then
				if _G.fileprotect(getCurrentScript() .. " Попытка переимовать/переместить библиотеки системы!") then return cp.invoke(address, method, ...) end
				return false
			else
				return cp.invoke(address, method, ...)
			end
		end
		...
		elseif method == "remove" then
			...
			if e[1] == "/OS.lua" then
				if _G.fileprotect(getCurrentScript() .. " Попытка удалить загрузчик системы!") then return cp.invoke(address, method, ...) end
				return false
			elseif e[1] == "/" then
				if _G.fileprotect(getCurrentScript() .. " Попытка очистить диск!") then return cp.invoke(address, method, ...) end
				return false
			elseif string.starts(e[1], "/Libraries/") then
				if _G.fileprotect(getCurrentScript() .. " Попытка удалить библиотеки системы!") then return cp.invoke(address, method, ...) end
				return false
			else
				return cp.invoke(address, method, ...)
			end
		...
		elseif method == "open" then
			...
			if e[1] == "/OS.lua" then
				if _G.fileprotect(getCurrentScript() .. " Попытка редактировать загрузчик системы!") then return cp.invoke(address, method, ...) end
				return nil		
			else
				return cp.invoke(address, method, ...)
			end
	...
	elseif component.proxy(address).type == "eeprom" and method == "set" then
		if _G.fileprotect(getCurrentScript() .. " Попытка перепрошить eeprom!") then return cp.invoke(address, method, ...) end
	else
		return cp.invoke(address, method, ...)
	end
end
