...
local function getTemporaryPath()
	local temporaryPath
	repeat
...
local temporaryPath = getTemporaryPath()
filesystem.write(temporaryPath, "")
GUI.bootRealTime = math.floor(filesystem.lastModified(temporaryPath) / 1000)
filesystem.remove(temporaryPath)

local function getTime()
	return GUI.bootRealTime + computer.uptime()
end

function GUI.virsignal(...)
	if (getTime() - GUI.lastvirtime) < 2 then
		GUI.lastvirtime = getTime()
		return GUI.lastvir
	end
	...
	workspace:addChild(GUI.textBox(x + image.getWidth(sign) + 2, y, textWidth, #lines, 0x1D1D1D, 0xE1E1E1, lines, 1, 0, 0)).eventHandler = nil
	local buttonWidth = 10
	local button = workspace:addChild(GUI.roundedButton(x + image.getWidth(sign) + textWidth - buttonWidth + 2, workspace.height - offset, buttonWidth, 1, 0x00FF00, 0xE1E1E1, 0xE1E1E1, 0x00FF00, "Запретить"))
	...
	local button2 = workspace:addChild(GUI.roundedButton(2 + image.getWidth(sign) + textWidth - buttonWidth + 2, workspace.height - offset, buttonWidth, 1, 0xFF0000, 0xE1E1E1, 0xE1E1E1, 0xFF0000, "Разрешить"))
